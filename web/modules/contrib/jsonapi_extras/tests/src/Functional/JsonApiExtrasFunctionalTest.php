...
class JsonApiExtrasFunctionalTest extends JsonApiFunctionalTestBase {
  ...
  /**
   * Test bulk operations form.
   */
  public function testBulkOperations() {
    $admin_user = $this->createUser([], 'test_admin_user', TRUE);
    $this->drupalLogin($admin_user);

    // Click the "disable all resources" button.
    $this->drupalGet('/admin/config/services/jsonapi/extras/bulk');
    $this->getSession()->getPage()->pressButton('edit-disable');
    // Check that all resource are disabled: the enabled table should not have
    // tbody cells.
    $this->drupalGet('/admin/config/services/jsonapi/resource_types');
    $this->assertSession()
      ->elementNotExists('css', '#jsonapi-enabled-resources-list tbody');

    // Click the "enable all resources" button.
    $this->drupalGet('/admin/config/services/jsonapi/extras/bulk');
    $this->getSession()->getPage()->pressButton('edit-enable');
    // Check that all resource are enabled: the disabled table should not have
    // tbody cells.
    $this->drupalGet('/admin/config/services/jsonapi/resource_types');
    $this->assertSession()
